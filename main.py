...
def retrieve_recents():
    ...
    c.execute('''SELECT DISTINCT conversations.page_ident, conversations.comment, conversations.conversation_time, CAST(threads.thread_time as integer) as time FROM conversations INNER JOIN threads ON threads.parent_time = conversations.conversation_time WHERE conversations.conversation_type = 'directory' ORDER BY time  DESC LIMIT 5''')
    for row in c:
